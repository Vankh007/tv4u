        ...
        .then(({ data, error }) => {
          if (error) {
            ...
            const mapped = (data || []).map((source) => ({
              ...
              version: source.version as "free" | "rent" | "vip",
              permission: source.permission as "Web & Mobile" | "Web" | "Mobile",
    ...
    if (access) {
      setVideoSources(prevSources => 
        prevSources.map(source => ({
          ...source,
          version: access as "free" | "rent" | "vip"
        }))
      );
    }
  }, [access]);
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error: episodeError } = await supabase
        .from("episodes")
        .update({
          access: access as "free" | "rent" | "vip",
        })
        .eq("id", episode.id);
