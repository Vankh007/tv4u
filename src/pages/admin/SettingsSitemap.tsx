  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (existing) {
        ...
        await supabase
          .from('site_settings')
          .insert({ setting_key: 'sitemap_xml', setting_value: settingValue });
      }
    ...
    <priority>0.9</priority>
      ...
      if (movies && movies.length > 0) {
        movies.forEach((item) => {
          ...
          generatedSitemap += `
  <url>
    <loc>${baseUrl}/watch/movie/${item.id}</loc>
    ...
    <priority>0.6</priority>
      ...
      if (series && series.length > 0) {
        series.forEach((item) => {
          ...
          generatedSitemap += `
  <url>
    <loc>${baseUrl}/watch/series/${item.id}</loc>
    <lastmod>${lastmod}</lastmod>
    <changefreq>weekly</changefreq>
    <priority>0.6</priority>
  </url>`;
      ...
      toast.success(`Sitemap auto-generated with ${(movies?.length || 0) + (series?.length || 0) + 3} URLs!`);
    } catch (error) {
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-yellow-500/10">
                  ...
                  <Badge className="bg-yellow-500 text-white">0.6-0.8</Badge>
                </div>
                <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-green-500/10">
                  ...
                  <Badge className="bg-green-500 text-white">0.3-0.5</Badge>
                </div>
              </CardContent>
            </Card>
          </div>
