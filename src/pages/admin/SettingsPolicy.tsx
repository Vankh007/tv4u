...
export default function SettingsPolicy() {
  ...
  const fetchPolicySections = async () => {
    try {
      ...
      data?.forEach(item => {
        const key = item.setting_key.replace('policy_', '');
        const value = item.setting_value as unknown as PolicySection;
        if (value && sectionsMap[key]) {
          sectionsMap[key] = { ...sectionsMap[key], ...value };
        }
      });
      ...
      toast.error('Failed to load policy sections');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (sectionKey: string) => {
    ...
    try {
      ...
      const { error } = await supabase
        .from('site_settings')
        .upsert({
          setting_key: `policy_${sectionKey}`,
          setting_value: JSON.stringify(section)
        }, { onConflict: 'setting_key' });

      if (error) throw error;

      toast.success(`${section.title} has been updated successfully`);
    } catch (error) {
